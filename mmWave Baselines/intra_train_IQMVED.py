...
if __name__ == '__main__':
    ...
    start = timer()
    loss_res = {'ecg': [], 'hr': [], 'rr': [], 'resp': [], 'align_loss': [], 'dist_loss': [],'all': []}
    ...
    with tqdm(range(max_iter + 1)) as it:
        for iter_num in it:
            ...
            ### IQ-MVED needs to output the distribution loss and alignment loss
            ecg_pre_zip, resp_pre_zip, HR_pr_zip, rf_pre_zip, dist_loss, align_loss = my_model(input, if_train=True)

            ecg_pre_aug_zip, resp_pre_aug_zip, HR_pr_aug_zip, rf_pre_aug_zip, dist_loss_aug, align_loss_aug = my_model(input_aug)
            ...
            if (iter_num > 0) and (iter_num % 1000 == 0):
                ...
                for step, (data, ecg, resp, HR_rel, RF_rel, _, _, _, _, _) in tqdm(enumerate(tgt_loader)):
                    ...
                    Wave_pr, Resp_pr, HR_pr, RF_pr, _, _ = my_model(data, if_train=False)
