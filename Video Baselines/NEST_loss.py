...
class NEST_TA(nn.Module):
    ...
    def Gaussian_Smooth(self, sample, mean, std=5):
        gmm_wight = torch.exp(-torch.abs(sample - mean) ** 2 / (2 * std ** 2)) / (
                    torch.sqrt(torch.tensor(2 * math.pi)) * std)
