...
def get_loss2(bvp_pre, hr_pre, bvp_gt, hr_gt, dataName, loss_sig, loss_hr, args, inter_num, spo_pre=None, spo_gt=None,
    ...
    if dataName == 'On-Road-rPPG':
        ...
        if loss_res is not None:
            loss_res['hr'].append(loss_hr(torch.squeeze(hr_pre), hr_gt).item())
            loss_res['spo'].append(loss_spo(torch.squeeze(spo_pre), spo_gt).item())
        loss = loss_hr(torch.squeeze(hr_pre), hr_gt) / 10 + k * (loss_spo(torch.squeeze(spo_pre), spo_gt) / 10) / 2
    elif dataName == 'VV100':
        if loss_res is not None:
            loss_res['bvp'].append(loss_sig(bvp_pre, bvp_gt).item())
            loss_res['spo'].append(loss_spo(torch.squeeze(spo_pre), spo_gt).item())
            loss_res['hr'].append(loss_hr(torch.squeeze(hr_pre), hr_gt).item())
        loss = loss_sig(bvp_pre, bvp_gt) + k * (loss_spo(torch.squeeze(spo_pre), spo_gt) + loss_hr(
            torch.squeeze(hr_pre), hr_gt)) / 2
    elif dataName == 'V4V':
        if loss_res is not None:
            loss_res['hr'].append(loss_hr(torch.squeeze(hr_pre), hr_gt).item())
            loss_res['rf'].append(loss_rf(torch.squeeze(rf_pre), rf_gt).item())
        loss = loss_hr(torch.squeeze(hr_pre), hr_gt) / 10 + k * (loss_rf(torch.squeeze(rf_pre), rf_gt) / 10) / 2
    elif dataName == 'HCW':
        if loss_res is not None:
            loss_res['bvp'].append(loss_sig(bvp_pre, bvp_gt).item())
            loss_res['hr'].append(loss_hr(torch.squeeze(hr_pre), hr_gt).item())
            loss_res['rf'].append(loss_rf(torch.squeeze(rf_pre), rf_gt).item())
        # loss_re, loss_res = loss_resp(resp_pre, resp_gt, loss_res, save_loss=False)
        # loss_res['resp'].append(loss_resp(resp_pre, resp_gt).item())
        loss = loss_sig(bvp_pre, bvp_gt) + k * (loss_hr(torch.squeeze(hr_pre), hr_gt) / 10 + (
                loss_rf(torch.squeeze(rf_pre), rf_gt) / 10)) / 2  # + loss_resp(resp_pre, resp_gt) / 10
