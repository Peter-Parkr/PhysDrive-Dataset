...
def rr_fft(sig, fs=30, harmonics_removal=True):
    """
    返回:
...
def get_args():
    ...
    parser.add_argument('-testper', '--test_percent', type=int, default=40,
    ...
    parser.add_argument('-tk', '--topk', type=float, default=10,
    ...
    parser.add_argument('-t', '--tgt', dest='tgt', type=str, default='On_Road_rPPG',
...
#         return y
#
#     lowcut = 0.1  # Lower bound of the respiratory rate in Hz (~6 BPM)
#     highcut = 0.5  # Upper bound of the respiratory rate in Hz (~30 BPM)
#     filtered_signal = bandpass_filter(bvp_signal, lowcut, highcut, fs)
#
#     # Use FFT to find the dominant frequency in the filtered signal
#     n = len(filtered_signal)
#     freqs = np.fft.rfftfreq(n, d=1 / fs)
#     fft_spectrum = np.abs(np.fft.rfft(filtered_signal))
#
#     # Find the peak frequency in the respiratory range
#     peak_index = np.argmax(fft_spectrum)
#     peak_freq = freqs[peak_index]
#
#     # Convert frequency to breaths per minute
#     rr = peak_freq * 60
#     return rr
def rr_cal(bvp_signal, fps=30):
